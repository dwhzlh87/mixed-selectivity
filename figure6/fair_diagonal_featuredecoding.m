...
pre_numpick=min([length(pre_LS_stimset),length(pre_NMS_stimset)]);
post_numpick=min([length(post_LS_stimset),length(post_NMS_stimset)]);
else
...
mode=3; %mode1 decode cue, model decode smaple, model decode match/nonmatch 

for r=1:10  %repeat time
    disp(r);
%temp_vec=randpick(pre_numpick); %use pre dataset 
%if cell_group==1
%temp_vec=randperm(length(pre_LS_stimset));             
%cells_to_use_stimset=pre_LS_stimset(temp_vec(1:pre_numpick));
%else
%temp_vec=randperm(length(pre_NMS_stimset));        
%cells_to_use_stimset=pre_NMS_stimset(temp_vec(1:pre_numpick));
%end
...
temp_vec=randpick(post_numpick); %use post dataset
if cell_group==1
temp_vec=randperm(length(post_LS_stimset));
cells_to_use_stimset=post_LS_stimset(temp_vec(1:post_numpick)); 
else
temp_vec=randperm(length(post_NMS_stimset));
cells_to_use_stimset=post_NMS_stimset(temp_vec(1:post_numpick)); 
end
%[x,y,crv_cell,population_response]=SVM_decoder3(0.4,0.1,cells_to_use_stimset,mode,stim_set); 
[x,y,crv_cell,population_response]=SVM_decoder3(0.4,0.1,cells_to_use_stimset+1553,mode,stim_set); 
store_mat(r,:)=y;
end
plot_mat=mean(store_mat,1);
save post_223_all.mat store_mat
save post_223_mean.mat plot_mat;
